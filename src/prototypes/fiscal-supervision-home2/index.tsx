                ...
                return (
                  <div key={category.id} className={`${getColSpanClass(span)} space-y-4`}>
                    <button
                      ...
                      className="group flex items-center gap-2 cursor-pointer transition-all hover:text-[#1456B8]"
                      title={`点击进入${category.name}工作台`}
                    >
                      <div className="w-0.5 h-3 bg-[#0F3D8A]/25 rounded-full group-hover:bg-[#1456B8] transition-colors" />
                      <div className="text-[12px] font-bold text-slate-500 tracking-wide group-hover:text-[#1456B8] transition-colors">
                        {category.name}
                      </div>
                      <span className="text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity text-sm">
                        ›
