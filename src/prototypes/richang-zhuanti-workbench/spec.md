...
- 页面底色继承 `ufsp-sky` 公共变量 `--ufsp-global-page-background`，与全局总览保持同一浅蓝径向+纵向渐变背景。
- 专题监控内容容器继续使用白底、轻阴影和紧凑信息密度，避免背景渐变干扰业务监控阅读。

## 4. 数据与交互

- 默认主题为 `special_monitor`，默认分类为 `category=daily`。
