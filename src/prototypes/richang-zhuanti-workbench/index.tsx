...
const Component = forwardRef<AxureHandle, AxureProps>(function Component(innerProps, ref) {
  ...
  const topicName = getConfigValue<string>(configSource, 'topic_name', String(query.topic || '专题监控通用工作台'));
  ...
  const topicKey = String(query.topic || 'special_monitor').toLowerCase();
  ...
  const defaultTodos: TodoItem[] = [
    ...
    // 第二组：问题整改更新
    { t: '督办三保监控整改任务', d: '03-12', level: '高', flowNode: '问题整改更新', module: '整改督办管理', status: '待处理', actions: ['处理'] },
  ...
  const topicProfile = useMemo(() => {
    ...
    return {
      displayName: '日常监督专题监控工作台',
      descText: '用于汇聚工资、三保、三公、一卡通、政采等专题监控任务，复用重点领域整改工作台的评价分析与趋势研判体验。',
      stageData: [
      ...
      evaluationIntro: '当前评价体系围绕日常监督专题监控构建，从整改进度、整改时效、责任落实、资金安全、治理成效、数据质量六个一级维度，对专题监督任务推进情况、问题闭环水平、资金规范性及治理改进成效进行综合分析与预警。',
  ...
  const selectedL2PolicyItems = useMemo(() => {
    ...
    const fallbackPolicyLink = useKeyAreaData
      ...
      : 'https://example.gov.cn/policy/daily-special-monitor-2025';
    return [
      ...
      { label: '政策条例', value: selectedL2.detail.policyClause || (useKeyAreaData ? '第十二条：建立专项债项目资金全流程监测及偏差整改机制。' : '第十条：建立专题监控指标预警、疑点核验和整改闭环机制。') },
