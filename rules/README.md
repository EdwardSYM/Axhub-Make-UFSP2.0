...
默认不要求用户逐项确认字号、尺寸、颜色和间距，也不为统一样式单独阻塞当前业务推进。执行者应先继承强基准和复用最多的稳定实现，在具体页面迭代中逐步积累确认结果；只有基线冲突、新组件 / 新变体或会覆盖其他已确认页面时才单独询问。可运行视觉样板作为可选校准工具，不作为每次新页面启动的必经步骤。
...
1. 先解决当前已确认范围内的问题。
2. 如果用户确认了某个样式或组件口径，判断它是当前页局部、页面范式还是组件规范。
3. 只在有必要时同步其他页面，且不得覆盖其他已确认页面冻结区。
4. 需要长期复用时，更新 `ufsp-page-governance.md`、`confirmed-baselines.md` 或对应 `spec.md`。
5. 如果规则稳定且反复使用，再考虑沉淀为项目专用 skill 或公共组件。
